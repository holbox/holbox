...
final class TestProject: XCTestCase {
    ...
    func testAddCardContent() {
        ...
        session.items[0]!.cards = [("world", ["hello"])]
    ...
    func testDeleteCard() {
        ...
        session.items[0]!.cards = [("", ["hello", "world", "lorem", "ipsum"])]
    ...
    func testNameSame() {
        session.items[0]!.time = .init(timeIntervalSince1970: 0)
        session.items[0]!.name = "hello world"
        store.session = { _ in XCTFail() }
        store.project = { _, _, _ in XCTFail() }
        session.name(0, name: "hello world")
        XCTAssertEqual(.init(timeIntervalSince1970: 0), session.items[0]!.time)
    }
    ...
    func testNameTrim() {
        session.items[0]!.name = "abc"
        session.name(0, name: "hello\nworld")
        XCTAssertEqual("helloworld", session.items[0]!.name)
    }
    
    func testListNameTrim() {
        session.items[0]!.cards = [("abc", [])]
        session.name(0, list: 0, name: "hello\nworld")
        XCTAssertEqual("helloworld", session.items[0]!.cards[0].0)
    }
}
